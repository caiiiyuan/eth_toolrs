...
pub async fn scan() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("rpc_provider_api: {}, telegram_apikey: {}, etherscan_apikey: {}", rpc_provider_api, telegram_apikey, etherscan_apikey);

    // Create a new http provider
    let provider = Provider::<Http>::try_from(rpc_provider_api)?; // replace with your Ethereum node URL
    ...
    while let Some(block) = stream.next().await {
        match block {
            Ok(block) => {
                ...
                let timestamp = block.timestamp.unwrap();
    ...
    // let api = Api::new(telegram_apikey); // replace with your Telegram bot token
    //
    ...
    //                     let etherscan_api_key = etherscan_apikey;  // Replace with your Etherscan API key
